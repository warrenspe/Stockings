...
class SelectStocking(threading.Thread):
    """
    ...
    def __repr__(self):
        return "<Stocking (Select) [%s]>" % str(self.addr)
    ...
    def __sendMessage(self):
        ...
        if not len(self._oBuffer) and not self._usIn.closed and self._usIn.poll():
    ...
    def run(self):
        ...
        try:
            ...
            while self.active:
                ...
                if len(self._oBuffer) or (not self._usIn.closed and self._usIn.poll()):
                ...
                if writable or len(self._oBuffer) or (not self._usIn.closed and self._usIn.poll()):
                    self.__sendMessage()
        ...
        # Caused by us being closed by our parent
